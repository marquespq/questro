...
import { useLeaderboard, Leaderboard, CurrentUserRank } from 'questro/leaderboard';
      ...
      <div style={styles.moduleDemo}>
        <div style={styles.demoCard}>
          <div style={styles.demoHeader}>
            ...
            <div style={styles.demoBadge}>Top {entries.length}</div>
          </div>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
              ...
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#86198f' }}>
                How Leaderboard Works
              </div>
            </div>
            <div style={{ fontSize: '13px', color: '#86198f', lineHeight: '1.6' }}>
              <strong>Score Syncing:</strong> Use &quot;Sync with Points&quot; to update leaderboard
              from points balance
              <br />
              <strong>Ranking System:</strong> Players sorted by score, rank updates automatically
              <br />
              <strong>Competition Stats:</strong> Track your rank, gap to #1, total score, and
              player count
            </div>
          </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <button onClick={syncScore} style={styles.actionButton}>
                🔄 Sync with Points
              </button>
              <button onClick={() => boostScore(50)} style={styles.actionButton}>
                ⚡ Boost +50
              </button>
              <button onClick={() => boostScore(100)} style={styles.actionButton}>
                🚀 Boost +100
              </button>
              <button
          ...
          {showAnimation && (
            ...
            >
              <div style={{ fontSize: '24px', marginBottom: '8px' }}>{getRankEmoji(userRank)}</div>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#15803d' }}>
                Score Updated!
              </div>
              <div style={{ fontSize: '12px', color: '#16a34a' }}>You are now rank #{userRank}</div>
            </div>
          )}

          <div style={styles.leaderboardContainer}>
            <Leaderboard entries={entries} currentUserId={userId} maxEntries={5} showAvatar />
          </div>
          ...
          <div style={{ marginTop: '24px', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              >
                <div style={{ fontSize: '28px', fontWeight: 800, color: getRankColor(userRank) }}>
              ...
              >
                <div style={{ fontSize: '28px', fontWeight: 800, color: '#6366f1' }}>
                  {currentUserEntry ? entries[0].score - currentUserEntry.score : 0}
                </div>
                <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>
                  Points to #1
                </div>
...
/>`}</pre>
        ...
        <div style={styles.codeBlock}>
          ...
          <pre style={styles.codeSnippet}>{`const { entries, updateScore } = useLeaderboard();
