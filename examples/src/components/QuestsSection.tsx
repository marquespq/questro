...
export function QuestsSection() {
  ...
  // Detecta quando uma quest é completada
  useEffect(() => {
    if (completedQuests.length > 0) {
      const lastCompleted = completedQuests[completedQuests.length - 1];
      if (lastCompleted.rewards.points && lastCompleted.completedAt) {
        const timeSinceCompletion = Date.now() - lastCompleted.completedAt;
        // Só mostra notificação se foi completada há menos de 5 segundos
        if (timeSinceCompletion < 5000) {
          setRewardNotification({
            title: lastCompleted.title,
            points: lastCompleted.rewards.points,
          });
  ...
  const simulateProgress = () => {
    if (activeQuests.length > 0) {
      ...
      if (objective) {
        // Incrementa o progresso atual (não sobrescreve!)
        const newProgress = objective.current + 1;
        updateProgress(quest.id, objective.id, newProgress);
  ...
  const completeAllObjectives = () => {
    if (activeQuests.length > 0) {
      ...
      quest.objectives.forEach((obj) => {
        if (!obj.completed) {
          // Completa o objetivo
          updateProgress(quest.id, obj.id, obj.target);
        }
      });
        ...
        <div className="demo-card">
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: '#92400e', lineHeight: '1.6' }}>
              <strong>Quest Types:</strong> One-time quests reset never, daily quests reset every
              day
              <br />
              <strong>Objective Tracking:</strong> Multi-step objectives track progress individually
              (0/5 actions)
              <br />
              <strong>Rewards:</strong> Earn points automatically when quest completes (added to
              balance)
            </div>
          </div>

          {/* Notificação de Quest Completada */}
          {rewardNotification && (
            <div
              style={{
                marginBottom: '24px',
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#a16207' }}>
                    {rewardNotification.title} (+{rewardNotification.points} points)
                  </div>
                </div>
                <div
                  style={{
          ...
          {activeQuests.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              ...
              <div className="demo-actions" style={{ gridTemplateColumns: '1fr 1fr' }}>
                <button onClick={simulateProgress} className="demo-action-button">
                  ➕ Make Progress (+1)
                </button>
                <button
            ...
            {availableQuests.length === 0 &&
              ...
              completedQuests.length === 0 && (
                ...
                >
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 600 }}>No quests available</div>
                  <div style={{ fontSize: '14px' }}>Check back later for new missions!</div>
                </div>
              )}
          </div>
        </div>
